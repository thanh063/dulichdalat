...
export function ChatWidget({ mode = "floating", autoSend = null }: ChatWidgetProps) {
  ...
  const quickChoices = useMemo(() => extractQuickChoices(mode), [mode]);
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      const data = (await response.json()) as AiResponse;
      setMessages((current) => [...current, { role: "bot", text: data.message }]);
      setChoices(data.choices ?? quickChoices);
  ...
  function handleChoice(choice: ChatChoice) {
    if (choice.payload.action === "go_node") {
      void sendMessage(choice.label);
    }
  }

  const panel = (
    <div className="flex h-full min-h-[32rem] flex-col overflow-hidden rounded-[2rem] border border-pine-500/10 bg-cream shadow-[0_24px_80px_rgba(26,47,15,0.18)]">
      ...
      <div ref={viewportRef} className="flex-1 space-y-4 overflow-y-auto bg-stone-50 p-4">
        {messages.length === 0 ? (
          <div className="rounded-[1.5rem] border border-pine-500/10 bg-white p-4 text-sm text-smoke">
            {greeting}
          </div>
        ) : null}
        ...
        {messages.map((message, index) => (
          ...
          >
            <div
              className={
                message.role === "user"
                  ? "rounded-[1.5rem] rounded-tr-none bg-pine-700 px-4 py-3 text-sm leading-7 text-cream"
                  : "rounded-[1.5rem] rounded-tl-none border border-pine-500/10 bg-white px-4 py-3 text-sm leading-7 text-charcoal"
              }
            >
        ...
        {isLoading ? (
          <div className="flex items-center gap-2 rounded-[1.5rem] rounded-tl-none border border-pine-500/10 bg-white px-4 py-3 text-sm text-smoke">
